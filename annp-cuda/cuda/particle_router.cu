 ...
 * 2. Optimized Single-Barrier Parallel Logit Computation.
 * 3. Race-Free Block-Level Reductions with Explicit __syncthreads Barrier Synchronization.
...
extern "C" __global__ void particle_router_kernel(
    ...
    const float* __restrict__ routing_table, // Transposed [num_neighbors, d_head]
    ...
    int tid = threadIdx.x; // 0..127 threads in parallel (4 warps per block)
    const float* pin = p_in + pid * d_head;
    const float* pout = p_out + pid * d_head;
    const float* gnoise = gumbel_noise + pid * num_neighbors;

    extern __shared__ float s_reduce_tmp[]; // Dynamic shared memory
    ...
    if (aligned) {
        ...
        for (int i = d_head4 * 4 + tid; i < d_head; i += blockDim.x) {
    ...
    float delta_p = sqrtf(s_reduce_tmp[0] + s_reduce_tmp[1] + s_reduce_tmp[2] + s_reduce_tmp[3]);
    __syncthreads(); // Barrier ensuring delta_p is read before next phase reuses s_reduce_tmp

    // 2. High-Performance Parallel Logits calculation with 16-byte Aligned Memory Offsets
    int num_neighbors_pad = (num_neighbors + 3) & ~3;
    float* s_logits = s_reduce_tmp + 4;
    float* s_probs  = s_logits + num_neighbors_pad;

    // Ensure s_reduce_idx starts at a 16-byte (4-float) aligned boundary
    int s_reduce_idx_offset = (4 + 2 * num_neighbors_pad + 3) & ~3;
    int* s_reduce_idx = reinterpret_cast<int*>(s_reduce_tmp + s_reduce_idx_offset);

    for (int k = tid; k < num_neighbors; k += blockDim.x) {
        const float* rt_k = routing_table + k * d_head;
        float dot = 0.0f;
        for (int d = 0; d < d_head; ++d) {
            dot += __ldg(pout + d) * __ldg(rt_k + d);
        }
        s_logits[k] = dot;
    }
    __syncthreads();

    // 3. Block-Level Parallel Max Reduction across ALL 128 threads
    ...
    float global_max = fmaxf(fmaxf(s_reduce_tmp[0], s_reduce_tmp[1]), fmaxf(s_reduce_tmp[2], s_reduce_tmp[3]));

    // 4. Parallel Softmax Exp & Sum Reduction
    ...
    float global_exp_sum = s_reduce_tmp[0] + s_reduce_tmp[1] + s_reduce_tmp[2] + s_reduce_tmp[3];
    float inv_sum = 1.0f / (global_exp_sum + 1e-8f);
    float noise_base = (num_neighbors > 0) ? (0.05f / (float)num_neighbors) : 0.0f;

    // 5. Parallel Entropy & Gumbel-Softmax evaluation across ALL 128 threads
    ...
    // Dynamic mask ensures shuffle safety across all threads
    unsigned int active_mask = __activemask();
    for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
        float other_max = __shfl_down_sync(active_mask, local_gumbel_max, offset);
        int other_k = __shfl_down_sync(active_mask, local_best_k, offset);
    ...
    if ((tid & 31) == 0) {
        ...
        (reinterpret_cast<float*>(s_reduce_idx))[tid >> 5] = local_gumbel_max;
        s_reduce_idx[4 + (tid >> 5)] = local_best_k;
    }
    __syncthreads();

    float global_entropy = s_reduce_tmp[0] + s_reduce_tmp[1] + s_reduce_tmp[2] + s_reduce_tmp[3];

    // Thread 0 collects warp results and writes final decision
    if (tid == 0) {
        float best_g_max = (reinterpret_cast<float*>(s_reduce_idx))[0];
        int best_k = s_reduce_idx[4];
        for (int w = 1; w < 4; ++w) {
            float w_gmax = (reinterpret_cast<float*>(s_reduce_idx))[w];
            if (w_gmax > best_g_max) {
                best_g_max = w_gmax;
                best_k = s_reduce_idx[4 + w];
            }
        }
...
struct RouterDeviceBufferPool {
    ...
    void ensure_capacity(size_t batch_size, size_t d_head, size_t num_neighbors, cudaStream_t stream) {
        if (batch_size > batch_cap) {
            ...
            cudaMallocAsync(&d_p_in, batch_cap * d_head * sizeof(float), stream);
            cudaMallocAsync(&d_p_out, batch_cap * d_head * sizeof(float), stream);
            cudaMallocAsync(&d_gnoise, batch_cap * num_neighbors * sizeof(float), stream);
            cudaMallocAsync(&d_chosen, batch_cap * sizeof(int), stream);
            cudaMallocAsync(&d_halting, batch_cap * sizeof(bool), stream);
            cudaMallocAsync(&d_headers, batch_cap * sizeof(ParticleCudaHeader), stream);
        }
        size_t rt_size = d_head * num_neighbors;
        if (rt_size > rt_cap) {
            if (d_routing_table) cudaFreeAsync(d_routing_table, stream);
            rt_cap = rt_size * 2 + 1024;
            cudaMallocAsync(&d_routing_table, rt_cap * sizeof(float), stream);
        }
    }
};
...
extern "C" void launch_particle_router(
    ...
    const float* dev_pin = get_device_ptr(p_in, g_router_pool.d_p_in, batch_size * d_head, stream);
    const float* dev_pout = get_device_ptr(p_out, g_router_pool.d_p_out, batch_size * d_head, stream);
    const float* dev_rt = get_device_ptr(routing_table, g_router_pool.d_routing_table, d_head * num_neighbors, stream);
    const float* dev_gnoise = get_device_ptr(gumbel_noise, g_router_pool.d_gnoise, batch_size * num_neighbors, stream);
    const ParticleCudaHeader* dev_hdr = get_device_ptr(headers, g_router_pool.d_headers, batch_size, stream);

    int* dev_chosen = get_device_ptr_mut(chosen_neighbor, g_router_pool.d_chosen, batch_size, stream);
    bool* dev_halting = get_device_ptr_mut(halting_flags, g_router_pool.d_halting, batch_size, stream);

    int threads_per_block = 128;
    int blocks = batch_size; // 1 block per particle
    int num_neighbors_pad = (num_neighbors + 3) & ~3;

    // Ensure total dynamic shared memory byte count accounts for 16-byte pointer alignment
    int s_reduce_idx_offset = (4 + 2 * num_neighbors_pad + 3) & ~3;
    size_t shared_mem_bytes = (s_reduce_idx_offset + 8) * sizeof(float);
