...
\begin{equation}
  ...
  \label{eq:routing}
\end{equation}
where $\mW_k \in \R^{d_\text{head}}$ is a fixed random projection for neighbor $k$
(not gradient-updated), $\mu_k$ and $\sigma_k$ are the empirical posterior mean and standard error
of ΔR credit observations along edge $k$, and $t_k$ is sampled from a Student-$t$ approximation
...
Each node maintains a temporal-difference state
$(\bar{\vp}_\text{last},\, \hat{\vp}_\text{pred},\, \ell_\text{last})$.
When a sub-batch for token $\ell > \ell_\text{last}$ arrives, the TD error is:
\begin{equation}
  \boldsymbol{\delta} = w(dt) \cdot \bigl(\hat{\vp}_\text{pred} - \bar{\vp}_\text{in}^\ell\bigr),
  \qquad w(dt) = \frac{1}{dt},\quad dt = \ell - \ell_\text{last}.
  \label{eq:tderror}
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \beta_d \cdot \bar{\Delta R} & \text{if } \bar{\Delta R} \le 0,
  \end{cases}
  \label{eq:credhealth}
\end{equation}
where $\bar{\Delta R}$ is the mean batch credit and $\beta_d = \texttt{negative\_credit\_damping}$
attenuates negative penalties to avoid premature death from transient noise.
...
The child's credit statistics start fresh (count$=0$), triggering guaranteed first evaluation
by the $+\infty$ prior score in \eqref{eq:thompson}.
