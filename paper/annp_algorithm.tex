...
The child is initialized with $h_\text{child,init} = h_\text{base}$, which is
intentionally below the 2-subnode neurogenesis threshold $h_\text{base}(1 + 2) = 3\,h_\text{base}$.
This structural inequality, $h_\text{base} < 3\,h_\text{base}$, prevents \emph{cascade neurogenesis}:
a child inheriting the parent's positive credit prior via Bayesian Thompson Sampling
would otherwise immediately trigger a further split upon first selection, proliferating untrained
subnodes to $K_\text{max}$ within a handful of batches.
An additional \emph{maturity guard} reinforces this:
neurogenesis requires $E_\text{cum,active} > 0$ (the candidate subnode must have processed
at least one input sub-batch). A freshly spawned child with $E_\text{cum} = 0$ cannot reproduce
regardless of its inherited credit statistics or health value.
The newborn's plastic phase is protected from premature death via the $\lambda$-scaled credit
penalty (Eq.~\ref{eq:credhealth}), which yields zero death penalty when $\lambda_{j} = 0$
(i.e.\ $E_\text{cum} = 0$), allowing gradient updates to adapt the perturbed weights toward
positive resonance before facing full Darwinian accountability.
