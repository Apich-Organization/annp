...
/// Parses SQLite database tables containing vector embeddings into sequence tensors [seq_len, d_model]
pub fn load_sqlite_dataset<P: AsRef<Path>>(
    ...
    let mut stmt = conn
        ...
        .map_err(|e| candle_core::Error::Msg(format!("SQLite Prepare statement error: {}", e)))?;
    ...
    for row in rows.flatten() {
        // Convert byte blob to f32 float vector
        ...
        if current_seq.len() == seq_len * d_model {
            let t = Tensor::from_vec(std::mem::take(&mut current_seq), (seq_len, d_model), device)?;
            tensors.push(t);
        }
    }
